import { Card, Cards } from "npm:scryfall-api";
...
async function* fetchPrints(search: string) {
  const sres = Cards.search(search, {
    unique: "prints",
    include_extras: true,
...
const prints: Card[] = [];
for await (
  const print of fetchPrints(
    '!"Forest" include:extras -layout:art_series game:paper',
  )
) {
  prints.push(print);
}
...
for await (
  const print of fetchPrints(
    '!"Snow-Covered Forest" include:extras -layout:art_series  game:paper',
  )
) {
  prints.push(print);
}
...
for await (
  const print of fetchPrints(
    '!"Forest" include:extras s:9ed l:ru -layout:art_series  game:paper',
  )
) {
...
const wcSets = new Set(
  prints.filter((c) => c.border_color === "gold").map((x) => x.set),
);
console.log("Fetching WC Deck owners...");
const wcSections = await Promise.all(
  Array.from(wcSets).map((set) => getWcSetData(set)),
);
...
const wcMap = new Map<string, Map<string, string>>(
  wcSections.map((playerCodes) => {
    ...
    return [
      set,
      new Map(playerCodes.map(({ id, name }) => ([id, name] as const))),
    ] as const;
  }),
);
...
function augmentCard(c: Card) {
  ...
  const sld_drop_name = c.set === "sld"
    ? sldSections.find((s) =>
      s.matches("sld", parseInt(c.collector_number, 10))
    )?.title
    : undefined;

  const signer = wcMap.get(c.set)?.get(
    c.collector_number.replace(/\d+.+/g, ""),
  );
  return {
    ...pick(c, [
      "object",
...
await Deno.writeTextFile(
  "forests.json",
  JSON.stringify(
    prints.map(augmentCard).sort((a, b) =>
